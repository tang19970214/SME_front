<template>
  <div style="width: 100%; height: 100%; overflow: hidden; position: relative;" ref="flowContainer">
    ...
    <draggable id="toDraggable" v-model="flowData.nodes" tag="div" v-bind="{group:'flow',animation: 200}" @end="handleMoveEnd" @add="handleAddFormItem" class="flow-container" style="cursor: pointer;" :class="{zoomIn: currentTool.type == 'zoom-in', zoomOut: currentTool.type == 'zoom-out', canScale: container.scaleFlag, canDrag: container.dragFlag, canMultiple: rectangleMultiple.flag }" :style="{ top: container.pos.top + 'px', left: container.pos.left + 'px', transform: 'scale(' + container.scale + ')', transformOrigin: container.scaleOrigin.left + 'px ' + container.scaleOrigin.top + 'px', 'z-index': isDrag ? 11 : -2 }">
    </draggable>
    <div id="flowContainer" ref="flowContainers" class="flow-container" style="cursor: pointer;" :class="{ grid: flowData.config.showGrid, zoomIn: currentTool.type == 'zoom-in', zoomOut: currentTool.type == 'zoom-out', canScale: container.scaleFlag, canDrag: container.dragFlag, canMultiple: rectangleMultiple.flag }" :style="{ top: container.pos.top + 'px', left: container.pos.left + 'px', transform: 'scale(' + container.scale + ')', transformOrigin: container.scaleOrigin.left + 'px ' + container.scaleOrigin.top + 'px' }" @click.stop="containerHandler" @mousedown="mousedownHandler" @mousemove="mousemoveHandler" @mouseup="mouseupHandler" @mousewheel="scaleContainer" @DOMMouseScroll="scaleContainer" @contextmenu="showContainerContextMenu">
      <template v-for="node in flowData.nodes">
        <flow-node :key="node.id" v-if="node && node.id" :node="node" :plumb="plumb" :select.sync="currentSelect" :selectGroup.sync="currentSelectGroup" :currentTool="currentTool" :isShowContent="isShowContent" @showNodeContextMenu="showNodeContextMenu" @isMultiple="isMultiple" @updateNodePos="updateNodePos" @alignForLine="alignForLine" @hideAlignLine="hideAlignLine">
        </flow-node>
      </template>
    ...
    <vue-context-menu :contextMenuData="containerContextMenuData" @paste="paste" @selectAll="selectAll" @saveFlow="saveFlow" @verticaLeft="verticaLeft" @verticalCenter="verticalCenter" @verticalRight="verticalRight" @levelUp="levelUp" @levelCenter="levelCenter" @levelDown="levelDown">
    </vue-context-menu>
    <vue-context-menu :contextMenuData="nodeContextMenuData" @copyNode="copyNode" @deleteNode="deleteNode">
    </vue-context-menu>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    addNewNode(node, index) {
      ...
      newNode.height = 50;
      if (
        newNode.type === "start round mix" ||
        newNode.type === "end round" ||
        newNode.type === "event" ||
    ...
    deleteNode() {
      ...
      const nodes = that.flowData.nodes;
      const lines = that.flowData.lines;
      that.flowData.lines = lines.filter(
        (link) =>
          link.from !== this.currentSelect.id &&
          link.to !== this.currentSelect.id
      );
