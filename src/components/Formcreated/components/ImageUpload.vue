<template>
  <div class="img-uplaod-container" :id="uploadId">
    <draggable class="drag-img-list" v-model="fileList" v-bind="{group: uploadId, ghostClass: 'ghost', animation: 200}" :no-transition-on-drag="true">
      <div :id="item.key" :style="{width: width+'px', height: height+'px'}" :class="{uploading: item.status=='uploading', 'is-success': item.status=='success', 'is-diabled': disabled}" class="upload-file" v-for="(item) in fileList" :key="item.key">
        ...
        <div class="uplaod-action" :style="{height: miniWidth / 4 + 'px'}">
          <i :class="item.isImg ? 'el-icon-view' : 'el-icon-download'" :title="item.isImg ? '預覽' : '下載'" @click="handlePreviewFile(item)" :style="{'font-size': miniWidth/8+'px'}"></i>
          <i v-if="isEdit && !disabled" class="el-icon-refresh" :title="'替換'" @click="handleEdit(item.key)" :style="{'font-size': miniWidth/8+'px'}"></i>
          <i v-if="isDelete && fileList.length > min && !disabled" class="el-icon-delete" :title="'刪除'" @click="handleRemove(item.key)" :style="{'font-size': miniWidth/8+'px'}"></i>
        </div>
      </div>
    </draggable>

    <div class="el-upload el-upload--picture-card" :class="{'is-disabled': disabled}" v-show="(!isQiniu || (isQiniu && token)) && fileList.length < length" :style="{width: width+'px', height: height+'px'}" @click.self="handleAdd">
    ...
    <Model :visible="previewVisible" @on-close="previewVisible = false" ref="formPreview" width="600px" form>
