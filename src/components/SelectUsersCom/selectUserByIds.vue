    ...
    <div class="flex-row" style="height: 100%;">
      ...
      <div class="flex-item table-box">
        <div class="flex-row" style="align-items: center;" v-if="loginKey === 'loginUser'" @keyup.13="handleSearchUser">
          ...
          <div style="text-align: right;padding: 5px 10px;" class="flex-item ellipsis">選中用戶：{{names}}</div>
        </div>
        ...
        <el-pagination background v-if="loginKey === 'loginUser'" layout="prev, pager, next" :total="tableData.total" :page-size="tableData.listQuery.limit" @current-change="handlePageSearch" style="margin-top: 15px;text-align: right;">
        </el-pagination>
      </div>
    </div>
...
export default {
  ...
  computed: {
    names() {
      ...
      if (
        ...
        this.tableData.selectTextsC.length > 0
      ) {
        names = [
          ...this.tableData.selectTexts,
          ...this.tableData.selectTextsC,
        ].join(",");
      }
      return names;
    },
  ...
  methods: {
    ...
    setSelectTable() {
      this.$nextTick(() => {
        const rows = [...this.tableData.datas].filter((x) =>
          [...this.tableData.selectIdsC].some((y) => y === x.id)
        );
        rows.forEach((row) => {
    ...
    // 確認用戶選擇
    handleSaveUsers() {
      let checkRoles = this.tableData.selectTextsC.filter(
        (it) => it !== "一般會員"
      );
      if (checkRoles.length >= 2) {
        this.$notify({
          title: "錯誤",
          message: "輔導顧問、創育單位、管理員 只能擇一",
          type: "error",
          duration: 2000,
        });
      } else {
    ...
    handleSelectionUser(val) {
      this.tableData.selectTextsC = val.map(
    ...
    getRoleList() {
      this.tableData.loading = true;
      this.tableData.listQuery.key = this.searchKey;
      roles.loadRoleByRule(this.listQuery).then((response) => {
