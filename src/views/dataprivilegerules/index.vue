<template>
  <div class="data-rule-wrap flex-column">
    ...
    <div class="app-container flex-item">
        ...
        <el-table ref="mainTable" height="calc(100% - 52px)" :key='tableKey' :data="list" v-loading="listLoading" border fit highlight-current-row
          ...
          <template v-for="(headerItem,index) in headerList">
            <el-table-column :label="headerItem.description" min-width="120px" :key="index">
              <template slot-scope="scope">
                <span v-if="headerItem.key === 'privilegeRules'" class="ruleSpan ellipsis" @click.stop="handleUpdate(scope.row)">{{scope.row[headerItem.key]}}</span>
          ...
          <el-table-column fixed="right" align="center" :label="'操作'" width="150" class-name="small-padding fixed-width">
            <template slot-scope="scope">
              ...
              <el-button v-if="scope.row.disable!=true" size="mini" type="danger"
                @click="handleModifyStatus(scope.row,true)">停用</el-button>
            </template>
          </el-table-column>
        </el-table>
      ...
      <el-dialog v-el-drag-dialog :destroy-on-close="true" class="dialog-mini rule-form-dialog" width="850px" :title="textMap[dialogStatus]"
          ...
          <el-row>
            ...
            <el-col :span="12">
              <el-form-item label-width="150px" size="small" :label="'资源标识（模块编号）'" prop="sourceCode">
                <!-- <el-input v-model="temp.sourceCode" @change="loadProperties"></el-input> -->
          ...
          <el-row>
            ...
            <el-col :span="4">
              <el-form-item size="small" :label="'是否可用'" prop="enable">
                <!-- <el-input v-model="temp.enable"></el-input> -->
      ...
      <!-- <el-dialog v-el-drag-dialog :destroy-on-close="true" class="dialog-mini" width="800px" title="权限规则"
        :visible.sync="dialogRuleVisible">
        <add-rule :dataSource="temp.privilegeRules" ref="addRule"></add-rule>
        <div style="text-align:right;margin-top: 10px;">
          <el-button size="small" type="cancel" @click="dialogRuleVisible = false">取消</el-button>
          <el-button size="small" type="primary" @click="handleSaveRules">确定</el-button>
        </div>
      </el-dialog> -->
    </div>
  </div>
  ...
  export default {
    ...
    methods: {
      ...
      handleDelete(rows) { // 多行删除
        dataPrivilegeRules.del(rows.map(u => u.id)).then(() => {
          this.$notify({
            title: '成功',
            message: '删除成功',
            type: 'success',
